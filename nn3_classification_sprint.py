...
initial_learning_rate_grid = [0.005]
final_learning_rate_grid = [0.001]
        ...
        for wl in lambdas:
            ...
            with graph.as_default():
                ...
                mix = '\n{} training hyper params: regularization: {}, learning rate: {}, final learning rate: {}, batch size: {}\n' \
                    .format('nn3_classification_sprint',
                ...
                learning_rate = tf.train.exponential_decay(initial_learning_rate, global_step, decay_rate=decay_rate,
                                                           decay_steps=num_epoch * num_steps / (
                ...
                def model(data):
                    ...
                    f2 = tf.nn.tanh(tf.nn.xw_plus_b(f1, weights2, biases2))
                ...
                with tf.Session(graph=graph) as sess:
                    ...
                    for epoch in range(num_epoch):
                        ...
                        for step in range(num_steps):
                            ...
                            if step % max(1, report_interval) == 0:
                                ...
                                if len(acc_history) > 1 and np.abs(acc_history[-1] - acc_history[-2]) < acc_epsilon:
                                    break
                    ...
                    ax1.set_ylim([0, max(1.0, np.max(loss_history) * 1.5)])
                    ax1.set_title('learning curve')
                    ax1.legend(('loss', 'accuracy'), loc='lower center')
                    ...
                    # plt.show()
                    time_consume = '\n{}, Cost time: {} min, regularization: {}, learning rate: {}, final learning rate: {}, batch size: {}\n'.format('nn3_classification_sprint', (time.time() - start_point) / 60, wl, initial_learning_rate, final_learning_rate, batch_size)
                    print(time_consume)
                    logger.write(time_consume)
                    topology = 'f1={}-f2={}'.format(f1_depth, f2_depth )
                    print(topology + '\n')
                    plt.savefig('datahouse/learning-curve-{}-{}-{}-{}-{}-'.format('nn3_classification_sprint', wl, initial_learning_rate, final_learning_rate, batch_size) + topology + '.png')
