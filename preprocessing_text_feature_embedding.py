...
def main():
    ...
    with graph.as_default():
        ...
        embed = tf.nn.embedding_lookup(embeddings,
                                       train_dataset)  # pick up vector with partition. shape=(batch_size, embedding_size)
        ...
        similarity = tf.matmul(valid_embeddings,
                               tf.transpose(
                                   normalized_embeddings))  # cosine similarity between the two vectors. shape=(num_examples, vocabulary_size): A[i ,j] means similarity between word i and word j.
